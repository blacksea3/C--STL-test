...
void BinarySearchTest()
{
	...
	for (int i = 0; i < 5000000; ++i) arr[i] = 2 * i;
	int count = 100000;
	cout << "recu binary search " << count << " times" << endl;
	estimateFuncTimeLoop(count, recuBSInterface, arr, 5001);
	cout << "iter binary search " << count << " times" << endl;
	estimateFuncTimeLoop(count, iterBSInterface, arr, 5001);
}
